...
def test_active_iq_config_advisor_one():
    ...
    for row in range(1, row_count):
        if ((data_read.get_cell_value('CA_product_names', row, colindex)) == 'test_cp_3_notifications') and ((data_read.read_by_col_name('CA_product_names', row, 'NetApp Chinese')) == properties_china('notifications_test_cp_3_string1')):
            expected_output = data_read.read_by_col_name('CA_product_names', row, 'NetApp Chinese')
...
def test_active_iq_config_advisor_two():
    ...
    for row in range(1, row_count):
        if ((data_read.get_cell_value('CA_product_names', row, colindex)) == 'test_cp_3_notifications') and (
                (data_read.read_by_col_name('CA_product_names', row, 'NetApp Chinese')) == properties_china(
