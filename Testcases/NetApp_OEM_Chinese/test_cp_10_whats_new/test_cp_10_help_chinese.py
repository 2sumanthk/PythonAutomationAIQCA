from functions.base_functions_zh import *
import pytest
import allure
from functions.ExcelFunctions import ReadWriteExcel

# filename to be used in screen shot
test_file_name = os.path.basename(__file__)


def test_whats_new_help_one():
    driver.get(properties_china('health_check_url'))
    maximize_window()
    sleep(2)
    click('help_product_title_help_button_xpath')
    sleep(2)
    actual_output = find_element("help_product_title_whats_new_xpath")
    expected_output = properties_china('check_point10_netapp_whats_new_string1')
    print("Element Returned Test: ", actual_output.text)
    highlight_element(actual_output)
    capture_screenshot(test_file_name)
    assert actual_output.text == expected_output


def test_verify_whats_new_link_two():
    actual_output = get_attribute("help_product_title_whats_new_xpath", "href")
    print("Link obtained is: ", actual_output)
    expected_output = properties_china('help_product_title_whats_new_link')
    assert actual_output == expected_output
    print('Link Verified Successfully')


def test_verify_whats_new_site_title_three():
    click("help_product_title_whats_new_xpath")
    switch_to_window(1)
    actual_output = title()
    expected_output = properties_china('verify_title_whats_new_string2')
    close_browser()
    assert actual_output == expected_output
    print('Title Verified Successfully')


# test_whats_new_help_one()
# test_verify_whats_new_link_two()
# test_verify_whats_new_site_title_three()