...
def test_header_one_english():
    actual_output = list(header_one())
    colindex = 1
    if data_read.get_cell_value('CA_product_names', 2, colindex) == 'product name':
        expected_output = data_read.read_by_col_name('CA_product_names', 2, 'NetApp English')
        assert expected_output == actual_output[0]


def test_header_two_english():
    actual_output = list(header_two())
    colindex = 1
    if data_read.get_cell_value('CA_product_names', 3, colindex) == 'product name':
        expected_output = data_read.read_by_col_name('CA_product_names', 3, 'NetApp English')
        assert expected_output == actual_output[0]


# test_header_one_english()
# test_header_two_english()
